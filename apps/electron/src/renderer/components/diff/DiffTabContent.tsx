...
export function DiffTabContent({ filePath, dirPath, sessionId, gitRoot, previewOnly, readOnly, basePaths, onEmptyDiff, toolbarActions, baseRef }: DiffTabContentProps): React.ReactElement {
  ...
  const openSideChat = useOpenSideChat({
    title: '预览选区问答',
    sourceKind: 'file',
    errorLogPrefix: 'DiffTabContent',
  })

  const handleOpenSelectionChat = React.useCallback(async (): Promise<void> => {
    if (!previewSelection) return
    // 来源展示标签取文件名（sourceRef 由 seed.filePath 自动推导）
    const fileName = previewSelection.filePath.split(/[\\/]/).filter(Boolean).pop() ?? previewSelection.filePath
    const ok = await openSideChat(sessionId, {
      text: previewSelection.text,
      sourceType: 'file',
  ...
  }, [clearPreviewSelection, openSideChat, previewSelection, sessionId])
